...
public class WatchlistServiceDefImpl implements WatchlistService {
    ...
    public Watchlist createWatchlist(Watchlist watchlist) {
        ...
        watchlist.setUsername(SecurityUtils.getPrincipalUsername(SecurityContextHolder.getContext().getAuthentication()));
        redisCacheService.deleteKeysByPattern("watchlists::" + watchlist.getUsername() + "*");
    ...
    public List<Watchlist> getWatchlistsByUsername(String userId, Pageable pageable) {
        log.info("Get watchlists for user ID: {}. Cache miss.", userId);
        Page<Watchlist> watchlists = watchlistRepository.findAllByUsername(userId, pageable);
    ...
    public Watchlist updateWatchlist(String id, Watchlist watchlist) {
        ...
        return watchlistRepository.findById(id).map(existingWatchlist -> {
